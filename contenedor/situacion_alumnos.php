<?php
    ...
    $procedureName="pSITUACION_ALUMNOS_"; 
    ...
    switch ($action) {
    	case 'REGISTRAR':
			...
			if (!empty($nombre)) {
				...
				$stmt1="CALL ".$procedureName."G(0, '$nombre', @id)";
    	...
    	case 'BUSCAR':
			...
			if ($result) {
				while($fila =$result->fetch_array(MYSQLI_BOTH)){
						$rows.=' <tr><td>'.$fila['ID_SITUACION_ALUMNO'].'</td><td>'.$fila['NOMBRE'].'</td></tr>';
		       }
			}
			$rows.='</tbody></table>';
			echo $rows;
